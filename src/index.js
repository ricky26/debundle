...
const bundleLocations = args._.concat(args.input, args.i);
const outputLocation = args.output || args.o;
const configPath = args.config || args.c;

if (!(bundleLocations.length && outputLocation && configPath)) {
...
let modules = [];
for (const bundleLocation of bundleLocations) {
  // ----------------------------------------------------------------------------
  // Read in bundle
  // ----------------------------------------------------------------------------
  console.log('* Reading bundle ' + bundleLocation + '...');
  ...
  let moduleAst = config.moduleAst.slice();
  let iifeModules = ast;
  let moduleAstPathTriedSoFar = [];
  while (true) {
    let operation = moduleAst.shift();
    if (!iifeModules) {
  ...
  if (config.type === 'browserify') {
    ...
    newModules = browserifyDecoder(iifeModules);
  } else {
    const webpackDecoder = require('./decoders/webpack');
    newModules = webpackDecoder(iifeModules, config.knownPaths);
  }
